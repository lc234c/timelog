...
/* 将 "YYYY-MM-DD" 解析为本地时区零点 Date，避免 Safari/iOS 把该格式当 UTC 导致日期偏移一天 */
function parseISODate(s) {
    var p = String(s).split('-').map(Number);
    if (p.length < 3 || isNaN(p[0]) || isNaN(p[1]) || isNaN(p[2])) return new Date(s);
...
var CHANGELOG = [
    { version: '1.7.3', date: '2026-08-26', tag: '修复', items: [
        '修复报表范围弹窗点「确定」无响应：加 try/catch + 报错 Toast，分享不支持时自动降级复制文本',
        '报表日期改用本地 midnight 解析，修复 Safari/iOS 下日期偏移一天导致统计漏算',
        '保留 1.7.2「从备份文件导入」功能，与下载备份形成完整闭环'
    ]},
...
function buildReport(range) {
    ...
    for (var d = parseISODate(range.start); d <= parseISODate(range.end); d.setDate(d.getDate() + 1)) {
...
document.getElementById('rptRangeConfirm').addEventListener('click', function () {
    try {
        ...
        if (mode === 'export') {
            var csv = res.rows.map(function (r2) { return r2.map(function (c) { return '"' + String(c).replace(/"/g, '""') + '"'; }).join(','); }).join('\r\n'), fn = '工时报表_' + range.label + '.csv';
            downloadFile(fn, '\uFEFF' + csv, 'text/csv;charset=utf-8'); showToast("📤 报表已开始下载");
        } else {
            var shareData = { title: '工时报表 ' + range.label, text: res.text };
            var doFallback = function () { fallbackCopyText(shareData.text); };
            if (navigator.share) {
                try { navigator.share(shareData).then(function () { showToast("✅ 分享成功"); }).catch(function (e) { if (e && e.name === 'AbortError') return; doFallback(); }); }
                catch (syncErr) { doFallback(); }
            } else { doFallback(); }
        }
    } catch (err) { console.error('报表确认回调出错:', err); showToast("❌ 生成报表失败：" + (err && err.message ? err.message : err), true); }
});
